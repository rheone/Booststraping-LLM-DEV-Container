...
| 🟡 Swimlanes | A process where step *ownership* matters as much as sequence | `swimlanes.md` |
...
```
````
**Default to Pattern C** when the target is an existing or new markdown document. If the diagram's own content needs to display a literal triple-backtick sequence (rare - e.g. documenting fenced output), bump the *outer* fence to 4 backticks so the inner one doesn't close it early.
...
8. When delivering diagrams into this skill's repo (e.g., adding new examples or fixing bugs), always run `node tools/validate-mermaid.mjs --mode parse` before committing.
